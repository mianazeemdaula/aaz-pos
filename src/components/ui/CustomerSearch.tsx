...
export function CustomerSearch({ value, onSelect, onCreateNew, placeholder = 'Search customer...', className, inputRef: externalInputRef }: CustomerSearchProps) {
  ...
  const search = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      const resp = await customerService.list({ q, pageSize: 10 });
  ...
  const pick = (c: Customer) => { onSelect(c); setQuery(''); setOpen(false); };
    ...
    return (
      <div className={`flex items-center gap-2 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-sm ${className ?? ''}`}>
        ...
        <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${value.balance > 0 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
          {value.balance > 0 ? `Due Rs ${value.balance.toLocaleString()}` : 'Clear'}
        </span>
        ...
        <div className="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-56 overflow-y-auto">
          ...
          {results.map(c => (
            <button key={c.id} type="button" onMouseDown={() => pick(c)}
              ...
              {c.balance !== 0 && <span className="text-xs shrink-0 text-red-600">Due Rs {c.balance.toLocaleString()}</span>}
            </button>
          ))}
          {results.length === 0 && !loading && <p className="px-3 py-2 text-sm text-gray-500">No customers found</p>}
        </div>
      )}
    </div>
  );
