...
export function QuickCustomerAdd({ open, onClose, onCreated }: QuickCustomerAddProps) {
    ...
    const save = async () => {
        ...
        setError('');
        try {
            const c = await customerService.create({ name: name.trim(), phone: phone.trim() || undefined, active: true, ...(openingBalance !== 0 && { openingBalance }) });
            reset();
            onCreated(c);
        } catch (e: unknown) {
