  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ml-auto ${supplier.balance > 0 ? 'bg-amber-100 text-amber-700' : supplier.balance < 0 ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>
          {supplier.balance > 0 ? `Payable ${fmt(supplier.balance)}` : supplier.balance < 0 ? `Advance ${fmt(Math.abs(supplier.balance))}` : 'Clear'}
        </span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          ...
          <dl className="space-y-1.5 text-sm">
            {supplier.phone && <div className="flex gap-1"><dt className="text-gray-500 w-24">Phone</dt><dd>{formatPhone(supplier.phone)}</dd></div>}
            {supplier.email && <div className="flex gap-1"><dt className="text-gray-500 w-24">Email</dt><dd>{supplier.email}</dd></div>}
            {supplier.city && <div className="flex gap-1"><dt className="text-gray-500 w-24">City</dt><dd>{supplier.city}</dd></div>}
            {supplier.ntn && <div className="flex gap-1"><dt className="text-gray-500 w-24">NTN</dt><dd>{supplier.ntn}</dd></div>}
          </dl>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
              ...
              {ledger.map((e, i) => (
                <div key={i} className="flex items-center justify-between text-xs border-b border-gray-100 dark:border-gray-700 pb-1.5">
                  ...
                  <div className="text-right">
                    {(e.debit ?? 0) > 0 && <p className="text-amber-600">+ {fmt(e.debit ?? 0)}</p>}
